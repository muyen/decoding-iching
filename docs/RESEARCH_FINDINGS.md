...
1. **純結構預測的理論極限：62.2%**（提升自60%）
2. **100%置信度規則** - 5條絕對規則被發現
3. **XOR模式關鍵** - xor=4在前4爻100%吉率
4. **字符-詞語方法有效** - 八卦視為字符，卦視為詞
5. **艮在下最吉** - 止為基，謙德之源
6. **兌在上最差** - 悅在上，輕浮
7. **得中比有應重要**
8. **五爻最穩定，六爻最危險**
9. **Gray碼穩定度** - 低Gray碼1數表示結構穩定
10. **模8餘4有利** - 發現模運算週期性
...
```
吉凶 = 結構(62.2%) + 語義(37.8%)
     = f(字符組合, 位置, 互卦, XOR, Gray) + g(卦名, 爻辭, 象徵)
```
...
*樣本數：90爻（15卦×6爻）*
*訓練最高準確率：64.4%（網格搜索）*
*交叉驗證準確率：58.9% ± 20.1%（真實泛化能力）*
*純結構合理估計：60-63%*
*方法：字符-詞語範式 + 深度逆向工程 + 2D網格分析 + 權重優化*
*技術：27種計算機科學逆向工程技術 + 網格/鄰居分析 + 遺傳算法/交叉驗證*
*注意：五行(五行)為後世附會，非原始結構，不使用*
