...
| 指標 | 數值 | 意義 |
|------|------|------|
| 結構預測準確率 | **~50%** | 僅靠結構可預測約一半（經5-fold CV驗證）|
| 需要爻辭的爻 | ~50% | 約一半爻需要閱讀爻辭 |
| 聚集效應 | 99.6% | 特殊爻幾乎全部聚集在一起 |

**注**：早期報告的39.8%是保守估計，後續優化達到~50%極限。
...
1. ~~**爻辭語義分析**：使用 NLP 深入分析爻辭內容~~ ✅ 已完成
2. **時間序列分析**：研究King Wen 序的數學特性
3. **交叉驗證**：使用其他易學傳統的解釋進行驗證
4. ~~**預測模型**：嘗試機器學習模型提高準確率~~ ✅ 已完成（極限~50%）

### 已完成的額外研究（2026-01-21）

- **3D立方體模型**：下卦性質×上卦性質×層次 = 27個狀態
- **五行分析**：相生最安全（凶率5.6%），比和最危險（凶率21.4%）
- **機器學習**：多種算法測試，5-fold CV驗證
- **結論**：純結構預測極限確認為~50%
...
- `experimental/scripts/` - 實驗性分析腳本（3D模型、五行、ML）

---

*分析日期：2026-01-21（更新）*
*分析工具：Python, Matplotlib, numpy*
*數據來源：周易六十四卦爻辭（完整384爻）*
*驗證方法：5-fold 交叉驗證*
